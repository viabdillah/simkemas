...
import { 
  ...
  LogOut, Users, LayoutDashboard, ShieldAlert, Database, Menu, X
} from 'lucide-react';
    ...
    <div className="min-h-screen flex bg-slate-50 font-sans text-slate-800 relative">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-white flex flex-col transform transition-transform duration-300 ease-in-out lg:static lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex-1 py-6 px-3 space-y-2">
          ...
          {/* Menu ini ACTIVE (Warna Biru) */}
          <button onClick={() => { setIsSidebarOpen(false); navigate('/super-admin/users'); }} className="w-full flex items-center gap-3 px-3 py-2.5 bg-blue-600 text-white rounded-md font-medium text-sm transition-colors cursor-pointer">
            <Users size={18} /> Kelola Pengguna (Role)
          </button>
          <button onClick={() => { setIsSidebarOpen(false); navigate('/super-admin/audit'); }} className="w-full flex items-center gap-3 px-3 py-2.5 text-slate-400 hover:bg-slate-800 hover:text-white rounded-md font-medium text-sm transition-colors cursor-pointer">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-16 flex items-center justify-between px-4 sm:px-6 bg-white border-b border-slate-200 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg sm:text-xl font-bold text-slate-800 truncate">Kelola Pengguna</h1>
          </div>
          <div className="flex items-center gap-3 sm:gap-4">
            <span className="text-xs sm:text-sm font-semibold text-slate-600 hidden sm:inline">👋 {currentUser?.username || 'Super Admin'}</span>
