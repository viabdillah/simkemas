...
export default function ManageUsers() {
  ...
  const [showPassword, setShowPassword] = useState(false); // Toggle Password Tambah
  ...
  const [showEditNewPassword, setShowEditNewPassword] = useState(false); // Toggle Password Baru Edit
  const [showEditConfirmPassword, setShowEditConfirmPassword] = useState(false); // Toggle Konfirmasi Edit
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok && result.ok) {
        toast.success(result.data?.message || "Data berhasil diperbarui");
        resetEditModal();
        handleRefresh();
      } else {
        toast.error("Gagal mengubah pengguna", { description: result.error });
      }
    } catch (err) {
  ...
  // Reset state modal edit sampai bersih termasuk toggle eye
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          <Button 
                            ...
                            title={u.id === currentUser?.id ? "Tidak bisa mengubah data sendiri" : "Ubah Data"}
                          >
      ...
      >
        <DialogContent className="sm:max-w-sm bg-white">
          <DialogHeader>
            ...
            <DialogDescription>Ganti hak akses atau reset kata sandi untuk <b>{editUser?.username}</b>.</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleEditSubmit} className="space-y-4 pt-2">
            ...
            {/* Input Kata Sandi Baru dengan Toggle Eye */}
            <div className="space-y-1">
              <label className="text-xs font-semibold text-slate-700">Kata Sandi Baru (Opsional)</label>
              <div className="relative">
                <Input 
                ...
                >
                  {showEditNewPassword ? <EyeOff size={16} /> : <Eye size={16} />}
                </button>
              </div>
            </div>

            {/* Input Konfirmasi Kata Sandi Baru dengan Toggle Eye */}
            {editNewPassword && (
              <div className="space-y-1">
            ...
            <DialogFooter className="pt-2">
              <Button type="button" variant="outline" onClick={resetEditModal} disabled={isEditing} className="cursor-pointer">
                Batal
              </Button>
              <Button 
                type="submit" 
                disabled={isEditing || (editRole === editUser?.role && !editNewPassword)} 
